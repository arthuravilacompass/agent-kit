...
Skills marcadas **slash-only** têm `disable-model-invocation: true` no frontmatter: rodam só via comando explícito (`/<plugin>:<nome>`), nunca por iniciativa do modelo (critério: `docs/SKILL-CONTRACT.md` §Critério slash-only).
