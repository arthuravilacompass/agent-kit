...
Six commands, all must come back green before any commit:
...
| `flutter-migration-lessons.md` | 12 depersonalized incidents from a native→Flutter migration pipeline's own retrospective, kept out of `apk-archaeology` on purpose — that skill covers the origin side of a migration (APK → catalog → backlog) and stops there; these are about the destination side (backlog → real Flutter code), a different boundary | Not yet a skill at all, only preserved raw material: a genuine downstream method needs a 2nd migrated capability to have completed all 4 phases before generalizing (only one has, twice — distilling from n=1 family would repeat the last lesson in the file). Promotion trigger: that 2nd capability, then design the actual skill around both. |
