...
- **Caixa obrigatória "O que isto NÃO é"**: não mede produtividade (sem baseline, sem migração real); fidelidade medida em referência limpa (NewPipe), não em código ofuscado (Telecorp é apêndice à parte); comportamento legado recuperado ≠ comportamento desejado aprovado (gate do PO permanece); **a Dimensão A foi demonstrada em referência pobre em regra de negócio (NewPipe é player de mídia, não tem pricing/entitlement/checkout/antifraude) — recuperação de regra de negócio real permanece projetada, não medida**; inferência de A pode errar mesmo em tier alto — o tier é calibrado, não garantido; **regra de negócio de baixa frequência é esquecida por design** (lição COBOL→Java, §11.1) — a spec candidata é insumo pra reconciliação humana, não substituto dela.
