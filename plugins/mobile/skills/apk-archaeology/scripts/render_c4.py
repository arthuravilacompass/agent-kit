#!/usr/bin/env python3
# desc: renders architecture.c4.mmd's numeric labels from findings.json + template (design §3)
"""render_c4.py — deterministic render layer for apk-archaeology (design §3).

Fills ONLY the {{ dotted.path }} placeholders in c4.template.mmd from the
values in findings.json (and would expand {{# map }}...{{/ map }} loop
blocks the same way render_overview.py does, if a future c4 template used
one -- none of the current template does). Sections wrapped in
%% AGENT:START name ... %% AGENT:END name are agent-authored annotations and
are copied through byte-for-byte, untouched.

This renderer is deliberately THIN: the C4 topology (native shell <-> WebView
via the bridge -> backend, + OIDC) is fixed by the template, not derived from
findings.json -- only the bridge-capability counts are numeric and synced.
findings.json's metrics.bridge_capabilities carries just the aggregate
{value, domains}, no per-domain breakdown, so a richer Component-level
diagram is not deterministically renderable from this contract; that detail
stays agent-authored (honest-scope rule).

Fails loud (non-zero exit) if a referenced deterministic path is null or
missing in findings.json.

Pure stdlib. Deterministic: same (findings.json, template) -> byte-identical
output every time.

Usage:
  python3 render_c4.py <findings.json> <template> --out <file>
"""
import argparse
import json
import re
...
AGENT_BLOCK_RE = re.compile(
    r"%%\s*AGENT:START\s+(\S+).*?%%\s*AGENT:END\s+\1\b",
    re.DOTALL,
)
...
def expand_loops(text, findings, stash):
    ...
    "{{ ... }}" text must render byte-for-byte, not be re-scanned/
    re-substituted by the later top-level scalar pass."""
...
def render(findings, template_text):
    ...
    # Protect agent-authored annotations and already-expanded loop rows
    # behind sentinel tokens -- neither is ever re-scanned by the top-level
    # scalar pass, even if their content happens to contain "{{" (defensive,
    # not just by luck).
    sentinels = {}
    ...
    # malformed template markup -- never agent-authored data. Fail loud
    # instead of letting it leak into the deliverable.
    if "{{" in rendered:
        raise RenderError(
...
def main():
    ...
    try:
        ...
        print(f"render_c4.py: error: {e}", file=sys.stderr)
        sys.exit(1)
