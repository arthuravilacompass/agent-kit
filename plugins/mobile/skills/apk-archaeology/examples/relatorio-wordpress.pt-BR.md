...
| **AUTH-03** | `send_login_email` + interceptação de link `magic-login` | `Authenticator.java:280` · `LoginMagicLinkInterceptActivity.java:24-38` | RF-03: o app permite entrar por link mágico enviado por e-mail | Entrar por link mágico | Alta | ☐ |
...
| **AUTH-06** | `UriLogin` extrai `user_login`/`password` de retorno de autorização | `ApplicationPasswordLoginHelper.java:125,687` | RF-06: o app permite conectar um site auto-hospedado via senha de aplicativo | Conectar site auto-hospedado (senha de aplicativo) | Alta | ☐ |
| **BUY-01** | busca com debounce 250ms + ordena relevância + oculta grátis | `DomainSuggestionsViewModel.java:172,436,389` | RF-07: o app permite buscar e selecionar um domínio disponível para registro | Buscar domínio | Alta | ☐ |
...
| US-AUTH-03 | Link mágico (e-mail) | `Authenticator.java:280` · `LoginMagicLinkInterceptActivity.java:24-38` | Alta |
| US-AUTH-04 | Passkey / chave de segurança (WebAuthn) | `Authenticator.java:109,113,252` | Alta |
| US-AUTH-05 | Criar conta (signup por e-mail) | `Authenticator.java:280` | Média-alta |
| US-AUTH-06 | Site auto-hospedado (senha de aplicativo) | `ApplicationPasswordLoginHelper.java:125,650-687` | Alta |
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | `POST https://public-api.wordpress.com/oauth2/token` com `username`, `password`, `grant_type=password` | `Authenticator.java:178-181` |
| 02 | Declara suporte a 2FA (`wpcom_supports_2fa=true`) e pede tipos de auth (`with_auth_types=true`) | `:182-183` |
| 03 | Resposta com `access_token` estabelece a sessão | `:151` |
| 04 | Credencial inválida → mensagem `"Incorrect username or password."` → `INCORRECT_USERNAME_OR_PASSWORD` | `:43, :356` |
| 05 | Mensagem de erro exibível vem do campo `error_description` do servidor | `:343` |

**Critérios de Aceite** 🟢
```gherkin
@legacy-observed @tier-alta   # PO ratifica
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | 2FA dispara com erro `needs_2fa` | `Authenticator.java:47` |
| 02 | Métodos suportados: **autenticador, backup, push, chave de segurança (webauthn)** | `:248-259` |
| 03 | Envia `wpcom_otp` + `get_bearer_token=true` | `:203-204` |
| 04 | Reenvio do código via `wpcom_resend_otp` (quando OTP vazio e reenvio pedido) | `:205-207` |
| 05 | Resposta traz `user_id` + um nonce por método | `:253-266` |
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | Envia e-mail de login via `send_login_email` (v1.3) | `Authenticator.java:280` |
| 02 | Payload: `email`, `client_id/secret`, `scheme` (padrão `WORDPRESS`), `flow`, `source`, `locale` | `:282-319` |
| 03 | Link aceito só se ação = `VIEW` **e** host contém `"magic-login"` | `LoginMagicLinkInterceptActivity.java:38` |
| 04 | Ao abrir, roteia para `WPMainActivity` com `ARG_IS_MAGIC_LINK_LOGIN=true` | `:24-33` |
| 05 | Variante Jetpack quando `?flow=jetpack` | `:41-49` 🟡 |
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | Desafio via `WebauthnChallengeRequest` | `Authenticator.java:109` |
| 02 | Token via `WebauthnTokenRequest` (após assinatura da chave) | `:113` |
| 03 | WebAuthn aparece como método 2FA (`two_step_nonce_webauthn`) | `:252` |
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | Cadastro por e-mail via `send_signup_email` (v1.1) | `Authenticator.java:280` |
| 02 | Payload inclui `signup_flow_name` quando presente | `:298-301` |

**Critérios de Aceite** 🟢
```gherkin
@legacy-observed @tier-media-alta
...
| RN | Regra | Evidência |
|---|---|---|
| 01 | A URI de retorno traz `user_login` e `password` nos parâmetros | `ApplicationPasswordLoginHelper.java:125` |
| 02 | Computa validade de `usuário/senha/siteUrl/apiRootUrl` (flags de vazio/nulo) 🟢 | `:650-653` |
| 03 | Rastreia se a URI já foi tratada (`alreadyProcessed`) — guarda de reprocesso **inferida** do log 🟡 | `:653` |
| 04 | Salva as credenciais de senha de aplicativo para o site | `:687` |
...
- ⬜ **A preencher pelo PO/negócio** *(template):* motivação do fluxo no super app · meta (ex.:
  conversão de registro) · muda algo do legado? · prioridade. `legacy-observed ≠ target-approved`
...
|---|---|---|
| Caminho feliz (buscar → ranquear → selecionar) | 🟢 preenchido | Cenários 1–3 |
| Regra de plano / carrinho | 🟢 preenchido | Cenários 4–5 |
| Busca vazia curto-circuita | 🟡 análogo | observado no fluxo **irmão** de gerenciamento (`newdomainsearch/NewDomainSearchViewModel.java:416-420`), não neste — confirmar |
| Erro de rede / timeout / retry | ⬜ **não observado** | não recuperável nesta partição → **decisão do time** |
| Estado "sem resultados" | ⬜ **não observado** | idem — a definir com PO/UX |
...
**Figma / layout** ⬜ — não existe no APK legado; design do app novo é do time de UX. *(slot: link do
Figma-alvo)*
...
- [ ] Implementado no Flutter conforme Figma-alvo
...
|---|---|---|
| US-AUTH-01 | 01–05 | §5.4 |
| US-AUTH-02 | 01–05 | §5.4 |
| US-AUTH-03 | 01–05 | §5.4 |
| US-AUTH-04 | 01–03 | §5.4 |
| US-AUTH-05 | 01–02 | §5.4 |
| US-AUTH-06 | 01–04 | §5.4 |
...
seção 7, como a 6, é só um índice de onde encontrar. Índice:
...
| CT-01, CT-02, CT-06 | RF-03 | US-AUTH-03 | Autenticação | `Authenticator.java:280` · `LoginMagicLinkInterceptActivity.java:24-38` | 5 | 2 | Alta | 🟢 envio + interceptação · ⬜ link expirado, rate-limit |
...
| CT-07 | RF-06 | US-AUTH-06 | Autenticação | `ApplicationPasswordLoginHelper.java:125,650-687` | 4 | 2 | Alta | 🟢 extração/validação/persistência · ⬜ descoberta do `apiRoot`; ponto cego = página web de autorização |
| CT-04, CT-08, CT-11, CT-12 | RF-07 | US-DOM-01 | Domínio | `DomainSuggestionsViewModel.java:172,436,389` | 7 | 5 | Alta (shell) | 🟢 happy + plano + carrinho · 🟡 busca vazia (análogo) · ⬜ erro de rede, sem resultados; ponto cego = checkout WebView |
...
Escala de status usada acima: 🟢 confirmado em código (e, quando indicado, em tela) · 🟡 inferido a
partir de comportamento indireto/análogo · ⬜ não observado — requer decisão/validação com o negócio.
...
| Tier / banda de confiança | Alta / Média-alta / Média / Baixa — calibrada, não garantida; uma RN de tier alto ainda pode estar errada |
| Ponto cego | Comportamento que roda fora do alcance da análise estática (dentro de WebView, em `.so` nativo, ou dependente de backend) |
