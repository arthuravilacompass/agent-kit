...
**Evidence** (source: tool-output): lines <lineStart>-<lineEnd> read via Read/Grep in this session — cite precisely, since the orchestrator manually spot-checks every citation against the actual file before presenting findings (the same discipline `core:grill-me` uses at `pre-done`) — an unread range must not become a finding.
