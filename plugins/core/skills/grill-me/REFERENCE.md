...
2. **Load context for the mode**
   ...
   **`pre-done`:**
   ...
   - If the plan's Key Decisions (brainstorm) or Key Technical Decisions (plan) carry `session-settled:` entries (CE's provenance annotation — `user-directed` or `user-approved`), load them. Epistemic class: these entries are the decision **contract**, same class as the ACs — not the author's narrative. Loading them does not violate the blind design.
