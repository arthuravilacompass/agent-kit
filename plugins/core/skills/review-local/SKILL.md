...
3. **Dispatch agents in parallel**
   ...
   **Dispatch:** all `Agent` calls in **a single message** (parallel). Each agent receives the diff (`git diff <base>...HEAD`) + the framing string from config. `consumer` receives ONLY the ticket text (resolved at runtime from the tracker tool available in the session — do not hardcode an MCP tool name) — never the diff. Every agent prompt MUST require citing `file:lineStart-lineEnd` precisely for any file cited as evidence — citations get spot-checked against the actual file before findings are presented (step 5).

4. **Wait for all agents to return.** Do not summarize partially.

5. **Aggregate findings**

   Before aggregating, **spot-check citations yourself** — the read-ledger hook and `citation-check.sh` are retired, so there's no mechanical gate anymore; manual spot-check replaces it:

   - For each finding, open the cited `file:lineStart-lineEnd` and confirm it actually shows what the claim asserts.
   - Citation confirmed → present normally.
   - Citation doesn't match — wrong lines, the file doesn't contain the claimed code, or the file/range doesn't exist → route to the "⚠️ Unverified" section, **not** presented as a confirmed finding.
   - While re-reading, also drop findings already fixed elsewhere in the diff.
   ...
   ### ⚠️ Unverified (<count>)
   - **[<agent-id>]** <finding> — <file>:<line> (citation didn't match the code on manual spot-check — possible fabrication)
