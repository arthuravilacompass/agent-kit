...
As linhas abaixo usam `pr-review-toolkit` (plugin de marketplace genérico) como roster default. As **framing strings** citam códigos de regra entre colchetes — substitua pelos códigos reais do projeto (ex.: os blockers de state-management/arquitetura definidos nas rules do seu stack). Sem essa config, o agente ainda roda, só que sem lista de códigos específicos para priorizar.

## Agents

| ID | Subagent type | Focus | Framing |
|---|---|---|---|
| `code` | `pr-review-toolkit:code-reviewer` | State-management, DI, Architecture smells (config: códigos de regra do projeto) | "[Códigos de regra blocker do projeto] violations must be flagged as 🔴 Important (not 🟡 Nit). Reference the rule code." |
| `silent` | `pr-review-toolkit:silent-failure-hunter` | Result-type misuse, try/catch sem propósito, swallowed errors | "Focus on: swallowed error branches in the project's error-handling type, try/catch without re-raise or state update, synchronous reset methods wrapped in try/catch." |
| `type` | `pr-review-toolkit:type-design-analyzer` | Hardcoded design-system values, boolean flags, raw discriminators (config: códigos de regra do projeto) | "Flag hardcoded visual values (colors, spacing, radius) that bypass the project's design-token system. Flag 3+ mutually-exclusive bool state fields as an implicit-FSM smell. Flag raw bool/String discriminators where a sealed/enum type would do." |
| `test` | `pr-review-toolkit:pr-test-analyzer` | Coverage em camadas críticas (config: alvo de %), testes de tela/componente para telas modificadas | "This project requires ≥[X]% coverage on [config: stores/repositories or equivalent]. Flag missing tests when a screen/component is modified." |
...
1. Add a row to the table above with a unique short ID, the `subagent_type`, the focus area, and the framing string (in pt-BR for guidance text; rule codes stay in whatever casing the project uses).
